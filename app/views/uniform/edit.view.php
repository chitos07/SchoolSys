        ...
        <div class="input_wrapper_other padding n20 select">
            <select  class="form-control" required name="StudentId">
                ...
                <?php if (false !== $student): foreach ($student as $students): ?>
                    <option value="<?= $students->Id ?>" <?= $this->selectedIf('school_student_id',$students->Id, $uniform ) ?>  > <?= $students->studnetName  ?> </option>
                <?php endforeach;endif; ?>
            </select>
        </div>
        <div class="input_wrapper n20 border">
            <label></label>
            <input class="form-control" placeholder="<?= $text_label_area ?>" required type="text" name="uniformPrice" maxlength="20" value="<?= $this->showValue('uniformPrice', $uniform) ?>">
        </div>
