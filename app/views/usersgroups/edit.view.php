<form autocomplete="off" class="appForm clearfix" method="post" enctype="application/x-www-form-urlencoded">
    <fieldset>
        ...
        <div class="input_wrapper n100">
            ...
            <input class="form-control" placeholder="<?= $text_label_group_title ?>"  required type="text" name="GroupName" id="GroupName" maxlength="20" value="<?= $groups->GroupName ?>">
        </div>
        <div class=" mt-3 list-group">
            <label><?= $text_label_privileges ?></label>
            <?php if ($privileges !== false) : foreach ($privileges as $privilege): ?>
                <label class="checkbox block">
                    <input type="checkbox" name="privileges[]" id="privileges"   <?=  in_array($privilege->PrivilegeId , $groupprivilegs) ? 'checked' : '' ?> value="<?= $privilege->PrivilegeId ?>">
